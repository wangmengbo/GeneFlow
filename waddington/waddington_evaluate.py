...
def main():
    ...
    if all_ssim:
        ...
        plt.axvline(ssim_mean, color='r', ls='dashed', lw=2)
        plt.title(f'SSIM (Mean: {ssim_mean:.4f})')
    else: 
        plt.title('SSIM Distribution (No data)')
    plt.xlabel('SSIM')
    plt.ylabel('Count')
    ...
    if all_psnr:
        ...
        plt.axvline(psnr_mean, color='r', ls='dashed', lw=2)
        plt.title(f'PSNR (Mean: {psnr_mean:.4f})')
    else: 
        plt.title('PSNR Distribution (No data)')
    plt.xlabel('PSNR (dB)')
    plt.ylabel('Count')
