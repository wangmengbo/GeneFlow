...
from dataset import CellAndPatchImageGeneDataset
...
def main():
    ...
    df = pd.read_csv(args.cell_patch_mapping)
    patch_to_cells = {}
    for k,v in df.groupby(df.columns[1]):
        patch_to_cells[k] = v[df.columns[0]].tolist()
        
    # Create dataset
    dataset = CellAndPatchImageGeneDataset(
        expr_df=pd.read_csv(args.gene_expr, index_col=0),
        patch_image_paths=args.image_paths,
